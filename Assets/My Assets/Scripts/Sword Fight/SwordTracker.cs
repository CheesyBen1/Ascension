...
public class SwordTracker : NetworkBehaviour
{
    ...
    void Update()
    {
        ...
        if (gameEnd == true)
        {
            ...
            foreach(GameObject player in players)
            {
                player.GetComponent<Sword>().hasSword = false;
            }
    ...
    public void updateLeaderBoard()
    {
        ...
        GameObject[] placements = players.OrderByDescending(p => p.GetComponent<PlayerStats>().health).ToArray();
        ...
        for (int i = 0; i < placements.Length; i++)
        {
            ...
            Score[i].GetComponent<TextMeshProUGUI>().text = placements[i].GetComponent<PlayerStats>().health.ToString("F0");
